...
export default function MainTabs() {
  ...
  return (
    <Tab.Navigator screenOptions={({route}) => ({
      headerShown: false,
      tabBarStyle: {backgroundColor: colors.tab, borderTopColor: colors.border, height: 62, paddingBottom: 8, paddingTop: 8},
      tabBarActiveTintColor: colors.primary,
      tabBarInactiveTintColor: colors.muted,
      tabBarLabelStyle: {fontSize: 12, fontWeight: '700'},
      tabBarIcon: ({focused}) => <Text style={{fontSize: focused ? 22 : 19}}>{iconFor(route.name)}</Text>,
    })}>
